...
function KeywordForm({
	...
	const { data: categoriesData, isLoading: categoriesLoading } = useOfflineKeywordCategories();
	const { data: naturesData, isLoading: naturesLoading } = useOfflineKeywordNatures();
	...
	const handleSubmit = (values: KeywordFormValues) => {
		if (frame.mode === "keyword-add") {
			...
			createMutation.mutate(payload, { onSuccess: () => { form.reset(); onClose(); } });
		} else if (keyword) {
	...
	return (
		<Stack gap="xs" p="xs">
			<form onSubmit={form.onSubmit(handleSubmit)}>
				<Stack gap="xs">
					<TextInput label={t("coloring.name")} {...form.getInputProps("name")} required />
					<Switch
						label={t("coloring.fullWordMatch")}
						checked={form.values.matchingType === "FULL"}
						onChange={(e) =>
							form.setFieldValue("matchingType", e.currentTarget.checked ? "FULL" : "PARTIAL")
						}
					/>
					{frame.mode === "keyword-add" && (
						<>
							<Select
								...
								allowDeselect={false}
								data={categoriesData?.map((cat: KeywordCategory) => ({ value: cat.id, label: cat.nameEn || cat.nameAr || "" }))}
								{...form.getInputProps("categoryId")}
								required
								leftSection={categoriesLoading ? <Loader size="xs" /> : <IconCategory size={16} />}
							/>
							<Select
								label={t("coloring.nature")}
								placeholder={t("coloring.selectNature")}
								allowDeselect={false}
								data={naturesData?.map((n: KeywordNature) => ({ value: n.id, label: n.nameEn || n.nameAr || "" }))}
								{...form.getInputProps("natureId")}
								required
								leftSection={naturesLoading ? <Loader size="xs" /> : <IconMasksTheater size={16} />}
							/>
							<TextInput label={t("coloring.description")} {...form.getInputProps("description")} />
						</>
					)}
					{createMutation.isError && (
						<Alert color="red">{t("coloring.createFailed")}: {createMutation.error?.message}</Alert>
					)}
					{updateMutation.isError && (
						<Alert color="red">{t("coloring.updateFailed")}: {updateMutation.error?.message}</Alert>
					)}
					<Group justify={frame.mode === "keyword-edit" ? "space-between" : "flex-end"} mt="md">
						{frame.mode === "keyword-edit" && !showDeleteConfirm && (
							<ActionIcon variant="transparent" color="red" size="lg" onClick={() => setShowDeleteConfirm(true)}>
								<IconTrash />
							</ActionIcon>
						)}
						<Group>
							<Button variant="outline" onClick={onClose}>{t("_.cancel")}</Button>
							<Button type="submit" loading={isPending}>{t("_.save")}</Button>
						</Group>
					</Group>
					{showDeleteConfirm && keyword && (
						<Alert color="red" variant="light">
							<Stack gap="xs">
								...
								<Text size="xs" c="dimmed">{t("coloring.deleteAliasesWarning")}</Text>
								<Group grow>
									<Button variant="outline" size="xs" onClick={() => setShowDeleteConfirm(false)} disabled={deleteMutation.isPending}>
										{t("_.cancel")}
									</Button>
									<Button color="red" variant="outline" size="xs" onClick={() => deleteMutation.mutate(keyword.id, { onSuccess: () => onClose() })} loading={deleteMutation.isPending}>
										{t("_.delete")}
									</Button>
								</Group>
							</Stack>
...
type AliasFormValues = { name: string; description: string; matchingType: "FULL" | "PARTIAL" };
...
function AliasForm({
	...
	return (
		<Stack gap="xs" p="xs">
			<Alert color="blue" variant="light" py="xs" styles={{ message: { fontSize: "var(--mantine-font-size-xs)" } }}>
				<Group gap="xs" wrap="nowrap">
					<Badge size="xs" color="blue" variant="filled">{t("coloring.alias")}</Badge>
					<Text size="xs" c="dimmed" style={{ flex: 1 }}>
						{t("coloring.aliasOf")}:{" "}
						<Text component="span" fw={600} size="xs" c="blue">{frame.parentKeyword.name}</Text>
					</Text>
				</Group>
			</Alert>
			<form onSubmit={form.onSubmit(handleSubmit)}>
				<Stack gap="xs">
					...
					<Group justify={frame.mode === "alias-edit" ? "space-between" : "flex-end"} mt="md">
						...
						<Alert color="red" variant="light">
							<Stack gap="xs">
								...
								<Group grow>
									...
									<Button color="red" variant="outline" size="xs" onClick={() => deleteMutation.mutate(alias.id, { onSuccess: () => onClose() })} loading={deleteMutation.isPending}>
										{t("_.delete")}
									</Button>
								</Group>
							</Stack>
...
function VersionForm({
	...
	const form = useForm<VersionFormValues>({
		initialValues: {
			...
			endingChapter: version?.endingChapter != null ? Number(version.endingChapter) : undefined,
		},
		validate: {
			categoryId: (v) => (!v ? t("home.categoryRequired") : null),
			natureId: (v) => (!v ? t("home.natureRequired") : null),
	...
	const handleSubmit = async (values: VersionFormValues) => {
		...
		if (imageFile) {
			...
			try {
				...
				setUploadError(error instanceof Error ? error.message : t("coloring.imageUploadFailed"));
				return;
			} finally {
		...
		if (frame.mode === "version-add") {
			...
			createMutation.mutate(payload, { onSuccess: () => { form.reset(); setImageFile(null); onClose(); } });
		} else if (version) {
			...
			updateMutation.mutate(
				{ id: version.id, data },
				{ onSuccess: () => { form.reset(); setImageFile(null); onClose(); } },
			);
		}
	};

	const startChapter = version ? Number(version.startingChapter) : (currentChapter ?? 0);
	const endChapter = version?.endingChapter != null ? Number(version.endingChapter) : undefined;

	return (
		<Stack gap="xs" p="xs">
			<Alert color="violet" variant="light" py="xs" styles={{ message: { fontSize: "var(--mantine-font-size-xs)" } }}>
				<Group gap="xs" wrap="nowrap">
					<Badge size="xs" color="violet" variant="filled">{t("coloring.version")}</Badge>
					<Text size="xs" c="dimmed" style={{ flex: 1 }}>
			...
			<Group grow>
				<NumberInput
					...
					value={isAdmin ? (form.values.startingChapter ?? startChapter) : startChapter}
					readOnly={!isAdmin}
					min={0}
					onChange={(v) => isAdmin && form.setFieldValue("startingChapter", typeof v === "number" ? v : undefined)}
				/>
				<NumberInput
					label={t("coloring.endingChapter")}
					value={isAdmin ? (form.values.endingChapter ?? endChapter ?? undefined) : (endChapter ?? undefined)}
					readOnly={!isAdmin}
					min={0}
					placeholder="∞"
					onChange={(v) => isAdmin && form.setFieldValue("endingChapter", typeof v === "number" ? v : null)}
				/>
			</Group>
			...
			<form onSubmit={form.onSubmit(handleSubmit)}>
				<Stack gap="xs">
					...
					<Group justify={frame.mode === "version-edit" ? "space-between" : "flex-end"} mt="md">
						...
						<Alert color="red" variant="light">
							<Stack gap="xs">
								...
								<Group grow>
									...
									<Button color="red" variant="outline" size="xs" onClick={() => deleteMutation.mutate(version.id, { onSuccess: () => onClose() })} loading={deleteMutation.isPending}>
										{t("_.delete")}
									</Button>
								</Group>
							</Stack>
...
export function ColoringForm({ frame, selectedNovelId, currentChapter, onClose }: ColoringFormProps) {
